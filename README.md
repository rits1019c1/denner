...
- **2D ゲームエンジン (Web限定):** `gui.setup`, `gui.rect`, `gui.loop` といった描画APIを搭載。ブラウザ上で直接 2D ゲームを開発・実行可能！
- **標準ライブラリ & インタラクティブ入力:** 充実のモジュール (`os`, `path`, `net`, `cli`, `gui`) を搭載。`cli.get_key()` により、ターミナルやブラウザを問わず矢印キー等によるリアルタイム操作が可能です。
